...
class LSTM_CNN(nn.Module):
    ...
    def __init__(self, input_size, lstm_input_size, num_lstm_layers, lstm_hidden_dim, out_size):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(in_channels=1,out_channels=32,kernel_size=(7,1),stride=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(3,4),stride=(3,1)),
            nn.Conv2d(in_channels=32,out_channels=32,kernel_size=(7,1),stride=1),
            nn.ReLU(),
        ...
        self.proj = nn.Sequential(
            ...
            nn.Linear(128,128), nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(128,lstm_input_size), nn.ReLU(),
        )
    ...
    def forward(self, x, lengths):
        B,H,W = x.shape
        x = x.reshape(B,1,H,W) # ignore sequences and run cnn on all images.
        features = self.cnn(x) 
        proj_feat = self.proj(features)# restore sequences
        # import pdb; pdb.set_trace()
        return self.sig(proj_feat)
        # lstm_out, _ = self.lstm(proj_feat)
        # out_space = self.out_layer(lstm_out).reshape(B,N,-1)
        # return out_space[torch.arange(len(lengths)),lengths-1]
